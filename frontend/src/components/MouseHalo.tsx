...
export default function MouseHalo() {
    ...
    // Check if we are on special screens
    const isSpecialScreen = pathname === "/" || pathname === "/login" || pathname === "/signup";

    // Lens size
    const lensSize = 40;
    // Center the system on the cursor
    ...
    useEffect(() => {
        ...
        // Only apply custom cursor on special screens (Home, Login, Signup)
        if (isSpecialScreen) {
    ...
    }, [mounted, isSpecialScreen, headX, headY, isVisible, lensOffset]);

    if (!mounted || !isSpecialScreen) return null;
    ...
    return (
        ...
        >
            {/* Soft Trail Smear - Always sized for magnifying lens now */}
